...
namespace Kroeg.EntityStore.Store
{
  public class RetrievingEntityStore : IEntityStore
  {
    ...
    private RelevantEntitiesService GtRE() => _relevantEntities = _relevantEntities ?? _serviceProvider.GetService<RelevantEntitiesService>();
    ...
    public async Task<APEntity> GetEntity(string id, bool doRemote)
    {
      ...
      {
        var headerContentType = string.Join(",", response.Content.Headers.FirstOrDefault(x => x.Key == "Content-Type").Value);
        if (converter.CanParse && ConverterHelpers.GetBestMatch(converter.MimeTypes, headerContentType) != null)
        {
          try
          {
    ...
    public async Task<APEntity> StoreEntity(APEntity entity)
    {
      ...
      return await Bypass.StoreEntity(entity);
    }
